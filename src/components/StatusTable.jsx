...
import { deleteStatus, statusDataById, updateStatus } from "../libs/apiHandler";

/* eslint-disable react/prop-types */
const StatusTable = ({ datas, loading, setAction }) => {
  if (loading) {
  ...
  const handleUpdate = async (id) => {
    const statusById = await statusDataById(id);
    const { isConfirmed } = await Swal.fire({
      ...
      <div>
        <div class="flex w-full items-center">
          ...
          <input id="swal-input1" class="swal2-input" value="${statusById.title}">
        </div>
      </div>
    `,
      inputAttributes: {
